...
export default function CitizenPortal() {
  ...
  const handleBooking = async (e) => {
    ...
    try {
      // Create token
      const res = await axios.post('/api/tokens/book/', {
        slot_id: selectedSlot.id,
        ...formData
      });
      setBookingResult(res.data);
      
      // Start payment flow instead of jumping to step 5 immediately
      await initiatePayment(res.data.token_id);
    } catch (err) {
              ...
              {services.map(s => (
                ...
                >
                  <div className="text-left">
                    ...
                    <p className="text-sm text-slate-500 mt-1 flex items-center gap-1"><Clock size={14}/> Est. {s.avg_duration_minutes} mins</p>
                  </div>
          ...
          <div className="animate-in fade-in slide-in-from-right-4 duration-500">
             ...
             <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {slots.map(s => (
                  <button
                    key={s.id}
                    disabled={s.is_full || s.is_blocked}
                    onClick={() => setSelectedSlot(s)}
                    className={`p-4 rounded-xl border text-center transition-all ${
                      s.is_full || s.is_blocked
                  ...
                  >
                    <div className="text-lg font-bold">{`${s.hour}:00`}</div>
                    {!(s.is_full || s.is_blocked) && (
                      <div className={`text-xs mt-1 font-medium ${selectedSlot?.id === s.id ? 'text-blue-100' : 'text-emerald-600'}`}>
                        {s.available_capacity} spots left
                      </div>
                    )}
                    {s.is_full && <div className="text-xs mt-1 font-medium text-red-500">Full</div>}
                    {s.is_blocked && <div className="text-xs mt-1 font-medium text-slate-500">Blocked</div>}
                  </button>
                ))}
            ...
            <div className="bg-slate-50 rounded-2xl p-8 mb-8 border border-slate-200 shadow-sm relative overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-4 text-left mt-6 pt-6 border-t border-slate-200">
                ...
                <div>
                  ...
                  <p className="font-bold text-slate-800">{bookingResult.date || selectedDate} at {bookingResult.hour || selectedSlot?.hour}:00</p>
                </div>
              </div>
            </div>
